...
const state = {
  ...
  konbiniStore: 'any',
  slotMode: 'standard',   // ポスト投函口の厚さ（既定=一般的 3.4cm）
  content: 'goods',
  expanded: false,
};

// ポスト投入口サイズのプリセット（厚さ・長辺 cm）
// ポスト投函口のプリセット（厚さ・長辺 cm）。投函口の厚さは地域・ポスト種別で差がある。
...
function updateKstoreVisibility() {
  const ks = $('konbini-store');
  if (ks) ks.classList.toggle('hidden', !state.places.konbini);
  // ポスト投函口の設定は「ポスト投函」を選んだときだけ表示
  const sb = $('slot-box');
  if (sb) sb.classList.toggle('hidden', !state.places.post);
}
